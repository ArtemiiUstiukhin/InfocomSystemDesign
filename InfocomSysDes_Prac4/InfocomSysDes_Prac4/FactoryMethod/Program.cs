...
namespace FactoryMethod {
    ...
    class DrunkDriver : TransportService {
        public double ServiseLevel { get; set; }
        public DrunkDriver(string name, int taff) : base(name) {
            ServiseLevel = taff;
        }
        public override double CostTransportation(double distance) {
            return distance * ServiseLevel;
        }
        public override string ToString() {
            string s = String.Format("Фирма {0}, уровень обслуживания {1}", Name, ServiseLevel);
            return s;
        }
    }
